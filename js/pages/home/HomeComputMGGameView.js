...
export default class HomeComputMGGameView extends Component {
  ...
  _listHomeHeaderComponent() {
    return (
      <View style={{ width: width, height: 285, backgroundColor: 'white' }}>
        {this._SearchMGGame()}
        {this._MoreMGGame()}
  ...
  _renderTuiJianMGgameItemView(item) {
    ...
    return (
      <TouchableOpacity
        ...
        key={item.index}
        activeOpacity={1}
        onPress={() => this._gotoShiWanActionGame(item,2)}>
        ...
        <TouchableOpacity activeOpacity={1} style={styles.tuijianStyle}
          onPress={() => this._gotoShiWanActionGame(item,2)}
        >
  ...
  _gotoShiWanActionGame(item,shiwanIndex) {
    const { navigate } = this.props.navigation;
    if (global.UserLoginObject.Token != '' || global.UserLoginObject.Token.length != 0) {
      ...
      paramsNotice.append("gameid", shiwanIndex==2?item.item.gameid:item.item.value.id);
        ...
        .then(response => {
          ...
          if (response.msg == 0) {
            if (global.OrientationLink == '1') {
              navigate('HomeDianZiGameView', { guest_url: response.data, title: shiwanIndex==2?item.item.game_name:item.item.value.game_name })
            } else {
  ...
  _renderMGgameItemView(item) {
    ...
    return (
      <TouchableOpacity activeOpacity={1} style={styles.cellStyle}
        onPress={() => this._gotoShiWanActionGame(item)}>
        <ImageCacheProvider
        >
