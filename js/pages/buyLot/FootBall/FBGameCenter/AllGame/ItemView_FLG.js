...
export default class ItemView_FLG extends Component {
    ...
    _centerVews(hvxKey, tit, startIdx) {
        ...
        let row = data[hvxKey] ? data[hvxKey].length / 2 : 0;
        let numColumn = 2;
        var viewArr = [];
            ...
            for (let b = 0; b < numColumn; b++) {
                ...
                if (data[hvxKey][idx]) {
                    if (this.props.d_key == 'WM') {
                        kTit = hvxKey != 'X' ? `净胜${data[hvxKey][idx]['k']}球` : (b == 0 ? `${data[hvxKey][idx]['k']}和局` : '任何进球和局');
                    } else if (this.props.d_key == '1X2FG' || this.props.d_key == 'DCFTS') {
                        kTit = (data[hvxKey][idx]['k'].indexOf('H') == 0 ? '主' : '客') + '(最先进球)';
                    } else {
                        kTit = data[hvxKey][idx]['k'] == 'H' ? '主' : data[hvxKey][idx]['k'] == 'V' ? '客' : data[hvxKey][idx]['k'] == 'o' ? '无进球' : '';
                    }
                }
                
                if (isSelect && this.handleSuperSlt) {
                    this._handleSuperSltDic(kTit, tit, hvxKey, data[hvxKey][idx]);
                }
    ...
    render() {
        ...
        if (this.props.d_key == 'FLG') {
            tit = '最先/最后进球';
            keyArr = ['FG', 'LG', 'NG'];
            titArr = ['最先进球', '最后进球', '无进球'];
        } else if (this.props.d_key == 'WM') {
            tit = '净胜球数';
        } else if (this.props.d_key == '1X2FG') {
            tit = '独赢 & 最先进球';
        } else if (this.props.d_key == 'DCFTS') {
            tit = '双重机会 & 最先进球';
            keyArr = ['HX', 'VX', 'HV'];
