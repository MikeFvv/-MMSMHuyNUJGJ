...
export default class ZRQiuItemView extends Component {
    ...
    render() {
        ...
        let leftW = (SCREEN_WIDTH - 21) * 0.15;
        ...
        return (
            <View style={[styles.itemBorderStyle, this.props.style]}>
                ...
                <View style={{ height: h, flexDirection: 'row' }}>
                    ...
                    <View style={{ height: h - 1, justifyContent: 'center', alignItems: 'center', borderColor: '#d2d2d2', borderRightWidth: 0.5 }}>
                        <View style={{ height: h * 0.5, width: leftW, justifyContent: 'center', alignItems: 'center', borderColor: '#d2d2d2', borderBottomWidth: 0.5 }}>
                            <Text allowFontScaling={false} style={{fontSize: Adaption.Font(17), color:'#313131'}}>全场</Text>
                        </View>
                        <View style={{ height: h * 0.5, width: leftW, justifyContent: 'center', alignItems: 'center' }}>
                            <Text allowFontScaling={false} style={{fontSize: Adaption.Font(17), color:'#313131'}}>半场</Text>
                        </View>
                    </View>

                    <ZRQiuItem style={{ height: h - 1, width: (SCREEN_WIDTH - 21) * 0.7 }}
                    ...
                    <TouchableOpacity activeOpacity={0.8} style={{ width: (SCREEN_WIDTH - 21) * 0.15, justifyContent: 'center', alignItems: 'center' }}
                        onPress={() => {
                            this.props.allPlayClick ? this.props.allPlayClick() : null;
                        }}
                    >
                        <Text allowFontScaling={false} style={{fontSize: Adaption.Font(18, 16), marginBottom: Adaption.Width(5), color:'#7e6b5a'}}>{`所有${'\n'}玩法`}</Text>
...
class ZRQiuItem extends Component {
    ...
    _centerVews() {
        ...
        for (let a = 0; a < row; a++) {
            ...
            let data = [];
            if (a == 0) {
                data = this.props.data['TG'];
            } else if (a == 1) {
                data = this.props.data['HTG'];
            }
                ...
                columnViews.push(
                    <TouchableOpacity key={idx} activeOpacity={0.8} 
                        style={{
                            ...
                            width: this.props.style.width / 4.0, height: height, 
                        ...
                        onPress={() => {
                            ...
                            if (isSelect) {
                                ...
                                let ballD = {'playMethod': a == 0 ? 'TG' : 'HTG', 'isFullTime': a == 0 ? true : false, sltIdx: idx};
                                Object.assign(ballD, data[b]);
                                dict[`${data[b].k}`] = ballD;
                            }
                        ...
                        {data != null
                            ? <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                <Text allowFontScaling={false} style={{ fontSize: Adaption.Font(16, 14), color: isSelect ? '#e33939' : '#313131', marginBottom: Adaption.Width(3) }}>{data[b]['k']}</Text>
                                <Text allowFontScaling={false} style={{ fontSize: Adaption.Font(14, 12), color: isSelect ? '#e33939' : '#676767' }}>{data[b]['p']}</Text>
                            </View>
                            : null
                        }
                    </TouchableOpacity>
