...
export default class PokerXianErBtn extends Component {
        ...
        if (nextProps.pokerBallsArr.length != 0){
            ...
            for (let i = 0; i < nextProps.pokerBallsArr.length; i ++){
                ...
                if (nextProps.pokerBallsArr[i]){
                    ...
                    let pokerObject = {pokerNum:pokerNumStr, pokerStyle:idxArr[2]};
    ...
    render () {
        ...
        return ( <TouchableOpacity activeOpacity={0.8} onPress = {() => {
            ...
            this.props.XianErBtnClick ? this.props.XianErBtnClick(!this.state.isClick,this.state.peilvData) : null;
        }} style = {this.props.style}>
            {SCREEN_WIDTH != 320 ? this._lineART([{x:0,y:170}, {x:(SCREEN_WIDTH - 110)/2,y:70}, {x:(SCREEN_WIDTH - 110)/2,y:0}, {x:0,y:0}]) : this._lineART([{x:0,y:140}, {x:(SCREEN_WIDTH - 110)/2,y:40}, {x:(SCREEN_WIDTH - 110)/2,y:0}, {x:0,y:0}])}
        </TouchableOpacity>)
