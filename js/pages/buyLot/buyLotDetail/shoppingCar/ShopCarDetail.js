...
export default class ShopCarDetail extends Component {
            ...
            <View style={{ marginLeft: 12, marginRight: 12, marginTop: 5, height: cellHeight, backgroundColor: '#fff' }}>
                ...
                <View style={{ paddingLeft: 10, height: 35, justifyContent: 'center' }}>
                    <CusBaseText numberOfLines={1} style={{ fontSize: Adaption.Font(17, 15), color: '#6a6a6a' }}>共
                        <CusBaseText style={{ color: '#e33939', fontSize: Adaption.Font(17, 15) }}> {item.item.value.zhushu} </CusBaseText>注
                        <CusBaseText style={{
                            color: '#828282',
                ...
                <Image style={{ height: 1, width: SCREEN_WIDTH - 44 - 20, marginLeft: 10, marginTop: 3 }} source={require('../../img/ic_dottedLine.png')} />
            </View>
        )
    }
        ...
        return (
            <View style={{
                ...
                <TouchableOpacity activeOpacity={1} style={{
                    flexDirection: 'row', width: WIDTH - Adaption.Width(18), alignItems: 'center', justifyContent: 'center',
                }} onPress={() => {
    ...
    _comformRequest() {
        ...
        } else if (parseInt(dataSource[0].value.qishu) < parseInt(global.CurrentQiShu)) {
        ...
        // 数据相同就不给投了。不然就投注重复了。
        if (this.tempComformData == dataSource) {
            console.log('相同了。不给投。。。');
            return;
        } else {
            console.log('不相同。开始投。。。。');
            this.tempComformData = dataSource;  // 把这个要提交投注的数据记下来。防止点多次了 投重复的。
        }
            ...
            <View>
                ...
                >
                    <View style={{ backgroundColor: COLORS.appColor, height: navHeaderHeight, flexDirection: 'row', justifyContent: 'space-between' }}>
                        ...
                        {/*中间标题或者view*/}
                        <CusBaseText style={[styles.imageLogoStyle]} adjustFontSizeToFit={true}>购物车</CusBaseText>
                    </View>
                    ...
                    <NormalHeader
                        ...
                        arcdomOneOnPress={() => {
                            ...
                            playDataModel.qishu = global.CurrentQiShu;
                    ...
                    >
                        <TouchableOpacity activeOpacity={1} style={{
                            ...
                            <TouchableOpacity activeOpacity={1} ref={(c) => this._Views = c} style={{
                                ...
                                <View style={{ height: Adaption.Height(48), justifyContent: 'center', alignItems: 'center' }}>
                                    <CusBaseText style={{ fontSize: Adaption.Font(18, 16) }}>{this.state.currentLookWanFaTitle}</CusBaseText>
                                </View>
    ...
    _xiangqingBallsTextView(ballsText) {
        ...
        for (let a = 0; a < ballsArr.length; a++) {
            ...
            textViews.push(
                <CusBaseText key={a} style={{ color: a % 2 == 0 ? '#7d7d7d' : '#e33939' }}>{string}</CusBaseText>
            );
        }
        return textViews;
    }
